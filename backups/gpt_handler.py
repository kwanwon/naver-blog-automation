...
"""  # 끝에 줄바꿈 두 개 추가하여 사용자 지침과 구분
        
        try:
            import json
            import os
            if os.path.exists('config/gpt_settings.txt'):
                with open('config/gpt_settings.txt', 'r', encoding='utf-8') as f:
                    user_settings = json.load(f)
                    # 기본 설정과 사용자 설정을 병합
                    default_settings.update(user_settings)
            
            # 사용자 지침 앞에 고정 검토 지침 추가
            if 'instructions' in default_settings:
                default_settings['instructions'] = fixed_review_instructions + default_settings['instructions']
            else:
                default_settings['instructions'] = fixed_review_instructions
                
            return default_settings
        except Exception as e:
            logger.error(f"GPT 설정 로드 중 오류 발생: {str(e)}")
            return default_settings

    def _load_custom_prompt(self):
        """저장된 커스텀 프롬프트를 로드합니다."""
        try:
            with open('config/custom_prompts.txt', 'r', encoding='utf-8') as f:
                prompts = f.read().split('---')
                if prompts:
                    return prompts[-1].strip()  # 가장 최근 프롬프트 사용
        except FileNotFoundError:
            return None
        except Exception as e:
            logger.error(f"프롬프트 로드 중 오류 발생: {str(e)}")
        return None
