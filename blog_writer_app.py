...
class BlogWriterApp:
    ...
    def _toggle_cafe_drive_watcher(self, e):
        ...
        if not self.cafe_watcher.is_running:
            ...
            if self.cafe_watcher.start():
                ...
                self.cafe_drive_watcher_btn.text = "감지 포스팅 중지"
            ...
            self.cafe_drive_watcher_btn.text = "감지 포스팅 시작"
    ...
    def _on_blog_drive_detected(self, folder_path, folder_name, files):
        ...
        def auto_upload():
            ...
            # [버그 픽스] 사진이 섞이거나 연달아 업로드될 때 누락되는 것을 방지하기 위해, 감지된 사진만 임시 폴더로 격리
            isolated_folder = os.path.join(get_app_data_dir(), "temp", f"blog_job_{int(time.time()*1000)}")
            os.makedirs(isolated_folder, exist_ok=True)
            isolated_files = []
            for fpath in files:
                if os.path.exists(fpath):
                    try:
                        dest = os.path.join(isolated_folder, os.path.basename(fpath))
                        shutil.move(fpath, dest)  # 원본 폴더에서 격리 폴더로 이동
                        isolated_files.append(dest)
                    except Exception as e:
                        print(f"⚠️ 파일 격리 중 오류 ({fpath}): {e}")
            ...
            try:
                ...
                # [리팩토링] 통합 파이프라인 호출 (격리된 폴더 전달)
                success = self._execute_unified_blog_posting(
                    page=self.page_ref,
                    topic=title,
                    content_str=content,
                    target_folder=isolated_folder,
                    reservation_time=None,
                    ai_tags=result.get('tags', []),
                    title=title,
                    is_detection_mode=False  # 사용자 요청 반영: 감지 모드에서도 이미지 처리(EXIF 제거, GPS 주입) 수행
                )
                
                if success:
                    print(f"✅ [DriveWatcher] 블로그 자동 포스팅 완료!")
                    self.file_manager.move_to_backup(isolated_files, folder_name)
                else:
                    print(f"❌ [DriveWatcher] 블로그 자동 포스팅 실패")
                    self.file_manager.move_to_error(isolated_files, "포스팅실패")
    ...
    def _on_cafe_drive_detected(self, folder_path, folder_name, files):
        ...
        def auto_upload():
            ...
            isolated_folder = os.path.join(get_app_data_dir(), "temp", f"cafe_job_{int(time.time()*1000)}")
            os.makedirs(isolated_folder, exist_ok=True)
            isolated_files = []
            for fpath in files:
                if os.path.exists(fpath):
            ...
            try:
                ...
                if success:
                    ...
                    self.file_manager.move_to_error(isolated_files, "카페포스팅실패")
    ...
    def _start_drive_auto_post(self, page):
        ...
        # UI 업데이트 로직 추가
        if hasattr(self, 'drive_auto_start_btn'):
            self.drive_auto_start_btn.bgcolor = ft.Colors.GREY_500
            self.drive_auto_start_btn.text = "감시 중..."
            self.drive_auto_start_btn.disabled = True
            
            self.drive_auto_stop_btn.bgcolor = ft.Colors.RED_700
            self.drive_auto_stop_btn.disabled = False
            
            self.drive_auto_status_text.value = "🟢 상태: 활발히 감시 중..."
            self.drive_auto_status_text.color = ft.Colors.GREEN_700
            page.update()
    ...
    def _stop_drive_auto_post(self, page):
        ...
        if self.drive_auto_post_system:
            ...
            # UI 업데이트 로직 추가
            if hasattr(self, 'drive_auto_start_btn'):
                self.drive_auto_start_btn.bgcolor = ft.Colors.TEAL_700
                self.drive_auto_start_btn.text = "🚀 자동 감지 시작"
                self.drive_auto_start_btn.disabled = False
                
                self.drive_auto_stop_btn.bgcolor = ft.Colors.GREY_500
                self.drive_auto_stop_btn.disabled = True
                
                self.drive_auto_status_text.value = "상태: 정지됨"
                self.drive_auto_status_text.color = ft.Colors.GREY_600
    ...
    def _execute_unified_blog_posting(self, page, topic, content_str, target_folder=None, reservation_time=None, ai_tags=None, title=None, is_detection_mode=False):
        """
        [핵심 통합 파이프라인]
        ...
        try:
            ...
            if images_available and custom_images_folder:
                try:
                    ...
                    # 🌟 [버그 픽스] 이전 작업의 임시/처리된 이미지 초기화 (엄청난 양의 사진 누적 방지)
                    image_handler.cleanup()
                    
                    # ImageHandler.process_and_watermark는 내부적으로 settings를 참조하므로 폴더 경로만 전달
                    processed_folder = image_handler.process_and_watermark(custom_images_folder)
                    if processed_folder:
                        custom_images_folder = processed_folder
                        print(f"✅ 이미지 후처리(워터마크/EXIF/GPS) 완료: {custom_images_folder}")
                except Exception as e:
                    print(f"⚠️ 이미지 후처리 중 오류 발생: {e}")
            ...
            try:
                if images_available and custom_images_folder:
                    from modules.news_poster.image_handler import ImageHandler
                    ImageHandler(self.settings).cleanup()
                    print("🧹 포스팅 완료 후 임시 이미지 보관함을 안전하게 비웠습니다.")
            except Exception as e:
                pass
...
"""
        ...
        watermark_path = ft.TextField(
            label="워터마크(엠블럼) 이미지 경로",
            hint_text="워터마크로 사용할 이미지 파일을 선택하세요",
            expand=True,
            data="watermark_path"
        )
        watermark_btn = ft.IconButton(
            icon=ft.Icons.IMAGE_SEARCH,
            tooltip="워터마크 이미지 선택",
            on_click=self._open_file_picker
        )

        watermark_size_percent = ft.TextField(
            label="워터마크 크기 비율 (%)",
            hint_text="예: 10 (기본값)",
            value="10",
            width=150
        ...
        gym_gps_coords = ft.TextField(
            label="체육관 GPS 좌표 (위도, 경도)",
            hint_text="예: 38.0772, 128.6185",
            expand=True
        )
        ...
        blog_slogan = ft.TextField(
            ...
            on_blur=lambda e: self._save_user_setting_individual('blog_slogan', e.control.value)
        )
        cafe_slogan = ft.TextField(
            label="카페 마지막 슬로건",
            hint_text="카페 글 마지막 슬로건",
            multiline=True, min_lines=2, max_lines=4,
            on_blur=lambda e: self._save_user_setting_individual('cafe_slogan', e.control.value)
        )
        band_slogan = ft.TextField(
            label="밴드 마지막 슬로건",
            hint_text="밴드 글 마지막 슬로건",
            multiline=True, min_lines=2, max_lines=4,
            on_blur=lambda e: self._save_user_setting_individual('band_slogan', e.control.value)
        )
        ...
        blog_first_sentence = ft.TextField(
            ...
            on_blur=lambda e: self._save_user_setting_individual('blog_first_sentence', e.control.value)
        )
        cafe_first_sentence = ft.TextField(
            label="카페 본문 첫 문장",
            hint_text="카페 포스팅 첫 문장",
            multiline=True, min_lines=2, max_lines=3,
            on_blur=lambda e: self._save_user_setting_individual('cafe_first_sentence', e.control.value)
        )
        band_first_sentence = ft.TextField(
            label="밴드 본문 첫 문장",
            hint_text="밴드 포스팅 첫 문장",
            multiline=True, min_lines=2, max_lines=3,
            on_blur=lambda e: self._save_user_setting_individual('band_first_sentence', e.control.value)
        )
        ...
        def save_user_settings(e, base_dir=None):
            try:
                ...
                settings = {
                    ...
                    "watermark_path": watermark_path.value,
                    "watermark_size_percent": watermark_size_percent.value,
                    "gym_gps_coords": gym_gps_coords.value,
                    "last_updated": datetime.now().strftime("%Y-%m-%d %H:%M:%S")
                }
            ...
            try:
                ...
                if os.path.exists(user_settings_path):
                    with open(user_settings_path, 'r', encoding='utf-8') as f:
                        ...
                        watermark_path.value = settings.get('watermark_path', '')
                        watermark_size_percent.value = settings.get('watermark_size_percent', '10')
                        gym_gps_coords.value = settings.get('gym_gps_coords', '')
        ...
        user_settings_tab = ft.Container(
            content=ft.Column(
                controls=[
                    ...
                    ft.Divider(),
                    ft.Text("🛡️ 워터마크 및 GPS 설정 (포스팅 이미지)", size=16, weight=ft.FontWeight.BOLD, color=ft.Colors.TEAL_700),
                    ft.Row([watermark_path, watermark_btn], spacing=10, vertical_alignment=ft.CrossAxisAlignment.CENTER),
                    watermark_size_percent,
                    ft.Row([gym_gps_coords, gps_auto_btn], spacing=10, vertical_alignment=ft.CrossAxisAlignment.CENTER),
        ...
        async def toggle_special_reservation(e):
            ...
            for row in special_time_rows_container.controls:
                ...
                if not re.match(r"^\d{1,2}:\d{2}$", start_time) or not re.match(r"^\d{1,2}:\d{2}$", end_time):
                    page.snack_bar = ft.SnackBar(content=ft.Text(f"시간 형식이 올바르지 않습니다 (HH:MM): {start_time} ~ {end_time}"))
                    page.snack_bar.open = True
            ...
            if not time_slots:
                page.snack_bar = ft.SnackBar(content=ft.Text("적어도 하나 이상의 시간대를 입력해주세요."))
                page.snack_bar.open = True
                special_enabled_checkbox.value = False
                page.update()
                return
        ...
        special_reservation_section = ft.Container(
            content=ft.Column([
                ...
                ft.Container(
                    content=special_time_rows_container,
                    padding=10,
                    bgcolor=ft.Colors.WHITE,
                    border_radius=5,
                    border=ft.border.all(1, ft.Colors.GREY_300)
        ...
        # ========== 드라이브 자동 포스팅 컴포넌트 초기화 ==========
        self.drive_auto_start_btn = ft.ElevatedButton(
            "🚀 자동 감지 시작",
            icon=ft.Icons.PLAY_ARROW,
        ...
        self.drive_auto_stop_btn = ft.ElevatedButton(
            "⏹️ 중지",
            icon=ft.Icons.STOP,
        ...
        self.drive_auto_status_text = ft.Text("상태: 정지됨", size=13, weight=ft.FontWeight.BOLD, color=ft.Colors.GREY_600)
            ...
            content=ft.Column([
                ...
                ft.Container(
                    content=ft.Column([
                        ...
                        # 시작/중지 버튼
                        ft.Row([
                            self.drive_auto_start_btn,
                            self.drive_auto_stop_btn,
                            self.drive_auto_status_text
                        ], spacing=10),
            ...
            content=ft.Column([
                ...
                ft.Container(
                    content=ft.Column([
                        ...
                        # 제어 버튼
                        ft.Row([
                            self.cafe_drive_watcher_status,
