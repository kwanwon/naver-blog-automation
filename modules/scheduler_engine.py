...
class SmartScheduler:
    ...
    def __init__(self, config_path: str):
        ...
        self.special_reservation_start = "09:00"
    ...
    def set_special_reservation(self, enabled: bool, start_time: str, end_time: str):
        """특별 예약 설정"""
        self.special_reservation_enabled = enabled
        self.special_reservation_start = start_time
        self.special_reservation_end = end_time
        print(f"📅 특별 예약 설정: {enabled} ({start_time} ~ {end_time})")
    ...
    def _special_reservation_monitor_loop(self):
        ...
        while self.special_reservation_enabled:
            ...
            try:
                # 시작/종료 시간 파싱
                start_h, start_m = map(int, self.special_reservation_start.split(':'))
                end_h, end_m = map(int, self.special_reservation_end.split(':'))
                
                start_dt = now.replace(hour=start_h, minute=start_m, second=0, microsecond=0)
                end_dt = now.replace(hour=end_h, minute=end_m, second=0, microsecond=0)
                
                # 현재 시간이 특별 예약 시간 범위 내인지 확인
                is_in_special_time = start_dt <= now <= end_dt
                
                if is_in_special_time and not self.special_reservation_running:
                    # 특별 예약 시간 시작!
                    print(f"\n🚨 [특별 예약] 시간 도달! ({self.special_reservation_start} ~ {self.special_reservation_end})")
