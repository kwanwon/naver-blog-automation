...
1. **[본문 구성]**: 오늘 하루를 정성껏 마무리하며 학부모님께 건네는 따뜻한 '마감 인사'와 함께, 삶의 지혜가 담긴 '명언'이나 '좋은 글귀'를 본문에 풍성하게 녹여내세요. 가르치는 태도가 아닌 따뜻한 위로와 응원의 메시지여야 합니다.
2. **[내일 예보]**: 글의 가장 마지막 문단(태그 바로 앞)에는 반드시 **'내일 아침 기상 정보'**를 상세히 안내하세요. "내일 아침 지역은 기온이 ~도까지 내려가고 바람도 차갑다고 하니, 아이들 등교길에 두툼한 겉옷 꼭 챙겨주세요"와 같이 부모님들이 실질적으로 대비하실 수 있도록 다정하고 정확하게 챙겨주며 마무리하세요.
⚠️ **[주의]**: 사용자 설정 주제(예: 낙법, 줄넘기 등)를 무시하고, '명언/좋은 글'과 '하루의 마무리'에만 집중하여 유익한 내용을 작성하세요.
⚠️ 할루시네이션 방지: 명언은 검증된 내용을 사용하되, 날씨 수치는 제공된 [실시간 날씨 상세 정보]를 절대적으로 따르세요.
"""
