...
class BandExpert(BaseAIExpert):
    ...
    def generate_band_content(self, topic, platform='band', task_type='regular', target_time=None, delta_days=0, **kwargs):
        ...
        if is_group_a:
            ...
            system_message = f"""당신은 아이들의 수련 과정을 애정 어린 시선으로 지켜보고 보고하는 [다정한 교육 전문가(지도자)]입니다.
{identity_rules}

[작성 목적: 유형 2 - 수련 현장 구체적 기록]
- 오늘 진행된 [수련내용]의 세부 종목들을 빠짐없이 자연스럽게 문장에 녹여내어 부모님께 보고하는 기록 글입니다.
- 너무 기계적이거나 딱딱한 보고서 형식을 피하고, 부모님과 대화하듯 편안하고 따뜻한 어조(~했습니다, ~했어요)를 섞어 완전한 문장으로 서술하세요.
...
- 문장이 너무 딱딱하게 끊기지 않도록, 부드럽게 이어지게 작성하세요. (단어만 나열하는 것은 절대 금지)
...
[필수 반영 사항]
- 제공된 [주제(수련내용)]에 적힌 운동 종목(예: 성장스트레칭, 구기 등)이나 단어들을 생략하지 말고, 반드시 "완전한 문장" 속에 자연스럽게 녹여내어 서술하세요. (단어만 쉼표로 나열하고 끝내는 것은 절대 금지)
- "오늘은 OOO, OOO 등을 수련하며 땀을 흘렸습니다" 등 구체적인 수련 내용이 포함된 서술형 문장으로 완성하세요. 단, "수련을 마쳤습니다" 처럼 글이 완전히 끝나는 듯한 종결 멘트는 쓰지 말고, 아이들의 수련 모습에 집중하여 서술하세요.

[출력 형식]
본문: 구체적 수련내용을 자연스럽게 엮은 1~2개 문단 (공백 포함 150~200자 이내, 반드시 완전한 서술형 문장으로 끝맺음).
...
"""
            ...
            if weather_hook:
                ...
                weather_instruction = f"""[System: 날씨 정보가 제공되었습니다!]
{weather_hook}

{tense_instruction}
🚨 [날씨 브리핑 강제 규칙 - 매우 엄격 ⭐⭐⭐⭐⭐]:
1. 데이터에 큰 일교차가 명시되지 않았는데 '일교차가 크니 감기 조심하세요'라고 하거나, 맑은 날씨에 '겉옷을 챙기세요'라는 식의 **근거 없는 상투적 인사말은 100% 원천 금지**합니다.
2. 오직 제공받은 날씨 팩트(기온, 하늘상태, 미세먼지, 강수확률 등)만을 바탕으로 단 1~2문장으로 아주 간결하게 브리핑하십시오.
3. [온도 표현 주의사항]: 기온이 10~20도 사이(예: 19도)일 경우 '포근하다' 또는 '따뜻하다'는 표현을 피하고, '선선한', '시원한' 또는 '살짝 쌀쌀한' 날씨로 정확하게 표현하세요. 강수확률과 함께 하늘 상태(맑음, 흐림 등)도 꼭 함께 언급하세요.
4. "요즘처럼 변화가 많은 날씨에는...", "면역력이 떨어질 수 있으니..."와 같이 날씨나 건강관리에 대한 불필요하고 장황한 훈계성 사족을 절대 쓰지 마시고, 간결하게 날씨 팩트만 전달한 뒤 즉시 본론으로 넘어가세요.
{weather_position_rule}
(작성 예시: {weather_example})
"""
            else:
                weather_instruction = """- 현재 상세 날씨 데이터가 없거나 일시적 통신 지연 상태입니다. 
- 절대 거짓 기온이나 날씨를 지어내어 쓰지 마세요(환각 방지). 대신 "우리 동네에도 맑고 쾌청한 공기가 감도는 날이네요" 등 계절과 절기에 걸맞은 정겹고 다정한 일반 안부 인사로 첫머리를 자연스럽게 시작하세요.
"""
