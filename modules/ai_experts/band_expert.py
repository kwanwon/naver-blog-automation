    ...
    def generate_band_content(self, topic, platform='band', task_type='regular', target_time=None, delta_days=0, **kwargs):
        ...
        if not is_group_a:
            ...
            if task_type == 'morning':
                ...
                topic = "하루를 위로하는 따뜻한 명언이나 조언 (다른 주제는 철저히 무시)"
...
- 본문의 알맹이가 끝난 후, 빈 줄을 하나 띄우고 독자가 집에서 1분 안에 손쉽게 실천할 수 있는 [1분 홈케어 팁]을 무조건 이어서 작성하세요.
- ⚠️ [필수 지정 팁]: 오늘은 무조건 **'{selected_tip['name']}'**({selected_tip['desc']})을(를) 주제로 서술하십시오.
- 주의: '[1분 홈케어 팁]' 등의 인위적인 머리말이나 대괄호 타이틀은 절대 달지 마세요.
- 본문의 내용과 자연스럽게 이어지는 연결 다리(Bridge) 멘트를 넣어 홈케어로 넘어가세요.
- 🚨 [말투 및 도입부 주의사항]:
  ...
  2. 도입부와 권유형 어미를 항상 새롭고 자연스럽게 변형하세요. (예: "해보는 것은 어떨까요?", "함께 1분만 투자해 보세요", "해보는 것도 좋겠네요", "가족이 함께 해보세요" 등 탄력적으로 작성)"""
            ...
            if task_type == 'closing':
                content_length_instruction = "공백 포함 100~150자 내외 (아주 짧고 간결하게 작성)"
            else:
...
[출력 형식 제한]
- **AI는 오직 시작인사 + 본문{home_care_output_instruction}만 작성합니다.**
- 본문의 순수 알맹이(인삿말과 홈케어를 제외한 핵심 본문) 길이를 {content_length_instruction}로 맞춰서 작성하세요.
- 최상단 고정 첫문장(인사말)과 하단의 체육관 슬로건 및 해시태그는 시스템이 자동 조립하므로 **절대 글에 포함하지 마세요.**
...
[출력 형식]
본문: 도입부 인사로 시작하여 유익한 정보와 공감을 전하는 모바일 최적화 본문(순수 본문 길이 {content_length_instruction}){home_care_output_append}.
