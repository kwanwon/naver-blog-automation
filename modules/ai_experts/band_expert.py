...
class BandExpert(BaseAIExpert):
    ...
    def generate_band_content(self, topic, platform='band', task_type='regular', target_time=None, delta_days=0, **kwargs):
        ...
        if is_group_a:
            # 📌 [유형 2] 생생하고 따뜻한 수련 현장 기록 지침 (자연스러운 톤, 기계적 어투 전면 배제)
            max_tokens = 600
            
            system_message = f"""당신은 아이들의 수련 현장을 애정 어린 시선으로 지켜보고 학부모님께 보고하는 [다정하고 전문적인 지도자]입니다.
{identity_rules}

[작성 목적: 유형 2 - 수련 현장 생생 기록]
- 오늘 진행된 [수련내용/부서]의 모습을 현장감 있고 따뜻하게 담아 학부모님께 전달하는 글입니다.
- 너무 짧고 판에 박힌 기계적 문장(예: "수련이 의미 있는 시간이 될 수 있다는 것을 느끼게 되었네요" 등 단순한 상투어)을 엄격히 금지합니다.
- 아이들이 집중하여 땀 흘리는 모습, 기본 동작과 기술을 하나씩 익혀가는 과정, 서로를 격려하며 활기차게 수련에 임하는 생생한 현장 분위기를 정성스럽게 묘사하세요.
- 모바일 밴드 화면에서 편안하게 읽을 수 있도록 **2~3개 자연스러운 단락(공백 포함 250~350자 내외)**으로 충분히 풍부하게 서술하세요.

[표현 지침 - 생생함과 따뜻한 진심 🌿]
- 과장된 미화("웃음꽃이 피었습니다", "송골송골 땀방울")나 딱딱한 사무적 보고체 대신, "힘찬 기합과 함께 집중하는 모습이 대견했습니다", "서로 격려하며 끝까지 동작을 완성해냈습니다"와 같이 현실적이고 훈훈한 현장 중심의 어조(~했습니다, ~했답니다)를 사용하세요.
- 수련 종목이나 부서의 특성(선수부/일반부 등)에 맞추어, 기본기 훈련, 체력 단련, 대련/기술 연습 등에서 보여준 열정과 성실함을 구체적으로 칭찬해 주세요.
- 🚨 스스로를 지칭할 때 '관장', '사범', '선생님' 등 특정 직급이나 호칭을 절대 사용하지 마세요. 대신 '지도자'라고 표현하거나, 호칭 없이 "더욱 성장할 수 있도록 정성껏 지도하겠습니다" 등으로 부드럽게 맺으세요.
...
- 🚨 [이모지 사용 절대 금지 ⭐⭐⭐⭐⭐]: 본문에는 어떠한 이모지(😊, 🙏 등)도 절대로 넣지 마세요. 오직 깔끔하고 정갈한 텍스트로만 작성하세요.

[공통 시스템 규칙]
{external_rules}
"""
            base_prompt = f"""[주제(수련내용): {display_topic}]
위 수련 내용을 바탕으로 학부모님께 생생한 현장 모습을 전하는 유형 2 본문을 작성하세요.

[필수 작성 지침]
- 단순한 한 줄 상투어("수련이 즐거운 시간이 되었습니다" 등)를 절대 쓰지 마세요.
- 오늘 수련에서 아이들이 진지하게 임한 태도, 동작 하나하나에 집중하며 흘린 땀방울, 서로 배려하고 격려하며 한 단계 성장한 모습을 **공백 포함 250~350자 분량(2~3개 단락)**으로 정성스럽고 매끄럽게 완성하세요.
- 제공된 [주제(수련내용)]가 있다면 해당 종목이나 핵심 내용을 문장 속에 자연스럽게 녹여내세요.

[출력 형식]
본문: 현장감과 온기가 느껴지는 2~3개 단락의 수련 기록 (공백 포함 250~350자 내외, 완전한 서술형 문장으로 끝맺음).
