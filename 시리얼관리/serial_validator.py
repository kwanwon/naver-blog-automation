...
try:
    ...
    # RotatingFileHandler: 최대 5MB, 백업 3개 유지
    ...
    logging.info(f"로깅 시작 (RotatingFileHandler), 로그 파일: {log_file}")
...
class SerialManager:
    def __init__(self):
        ...
        self.context_menu.add_separator()
        self.context_menu.add_command(label="디바이스 정보 초기화", command=self.reset_device_info)
    ...
    def backup_database(self):
        ...
        try:
            ...
            # 오래된 백업 자동 정리 (30일 이상, 최소 3개 유지)
            deleted_count = self.cleanup_old_backups(backup_dir, max_age_days=30, keep_minimum=3)
            
            self.backup_to_github()
            
            cleanup_msg = f"\n(오래된 백업 {deleted_count}개 정리됨)" if deleted_count > 0 else ""
            messagebox.showinfo("성공", f"데이터베이스가 백업되었습니다.\n위치: {backup_file}{cleanup_msg}")
        except Exception as e:
            logging.error(f"데이터베이스 백업 오류: {e}")
            messagebox.showerror("오류", "데이터베이스 백업 중 오류가 발생했습니다.")
    
    def cleanup_old_backups(self, backup_dir, max_age_days=30, keep_minimum=3):
        """오래된 백업 파일 자동 정리"""
        try:
    ...
    def edit_expiry_date(self):
        ...
        edit_window.geometry("520x220")
        edit_window.resizable(False, False)
        
        date_frame = ttk.LabelFrame(edit_window, text="만료일 선택")
        date_frame.pack(padx=10, pady=5, fill=tk.X)
        
        # 현재 선택된 시리얼의 만료일 가져오기
        current_expiry = ""
        if selected:
            item = selected[0]
            values = self.tree.item(item)['values']
            if len(values) > 3 and values[3]:
                current_expiry = str(values[3])
        
        # 직접 입력 방식 (더 안정적)
        input_frame = ttk.Frame(date_frame)
        input_frame.pack(pady=10, fill=tk.X, padx=10)
        
        ttk.Label(input_frame, text="만료일 (YYYY-MM-DD):").pack(side=tk.LEFT, padx=5)
        date_var = tk.StringVar(value=current_expiry if current_expiry else datetime.now().strftime('%Y-%m-%d'))
        date_input = tk.Entry(input_frame, textvariable=date_var, width=12, font=('Helvetica', 12))
        date_input.pack(side=tk.LEFT, padx=5)
        ...
        # 무제한 버튼 (개발자용)
        ttk.Button(period_frame, text="무제한", command=apply_unlimited).pack(side=tk.LEFT, padx=2)
        
        def update_expiry():
            # 텍스트 입력 필드에서 날짜 가져오기 (우선)
            new_date = date_var.get().strip()
            
            # 날짜 형식 검증
            try:
                validated_date = datetime.strptime(new_date, '%Y-%m-%d').date()
                new_date = validated_date.strftime('%Y-%m-%d')
            except ValueError:
    ...
    def create_serial(self):
        ...
        create_window.geometry("520x350")
        ...
        # 직접 입력 방식 (macOS 호환성)
        input_row = ttk.Frame(date_frame)
        input_row.pack(pady=10, fill=tk.X, padx=10)
        
        # 기본값: 오늘 + 180일
        default_date = (datetime.now().date() + timedelta(days=180)).strftime('%Y-%m-%d')
        ttk.Label(input_row, text="만료일 (YYYY-MM-DD):").pack(side=tk.LEFT, padx=5)
        date_var = tk.StringVar(value=default_date)
        date_input = tk.Entry(input_row, textvariable=date_var, width=12, font=('Helvetica', 12))
        date_input.pack(side=tk.LEFT, padx=5)
        ...
        def validate_and_create():
            try:
                ...
                progress_var.set("시리얼 생성 중...")
                create_window.update()
                
                serial = str(uuid.uuid4())
                
                # 🔧 수정: 시리얼 생성 시 디바이스 정보는 비어있음 (사용자가 등록할 때 저장됨)
                # 개발자 디바이스 정보가 아닌 빈 정보로 생성
                device_info = {}  # 비어있는 디바이스 정보
                
                # 🔧 수정: 초기 상태는 항상 "사용가능" (아직 사용자가 등록하지 않은 상태)
                initial_status = "사용가능"
                ...
                response = requests.post(
                    ...
                    json={
                        ...
                        "memo": memo_entry.get(),
                        "device_info": device_info  # 빈 디바이스 정보
                    },
                    timeout=30
                )
                ...
                if response.status_code == 200:
                    ...
                    ''', (
                        ...
                        json.dumps(device_info),  # 빈 디바이스 정보
                        memo_entry.get(),
                        0
                    ))
                    self.conn.commit()
                    ...
                    text_widget.insert(tk.END, f"메모: {memo_entry.get() or '없음'}\n\n")
                    text_widget.insert(tk.END, "─" * 40 + "\n")
                    text_widget.insert(tk.END, "📌 사용자가 이 시리얼을 등록하면\n")
                    text_widget.insert(tk.END, "   해당 사용자의 디바이스 정보가 저장됩니다.\n")
        ...
        try:
            ...
            try:
                ...
                if response.status_code == 200:
                    ...
                    logging.info(f"서버에서 {len(server_data)}개의 시리얼 데이터 가져옴")
                else:
                    logging.warning(f"서버 응답 오류: {response.status_code}")
            except Exception as e:
                logging.warning(f"서버 연결 실패: {e}")
            
            # 서버 연결 실패 시 로컬 DB 사용
            if server_data is None:
                logging.info("로컬 DB에서 데이터 로드")
                self.cursor.execute("""
                    SELECT serial_number, status, created_date, expiry_date, 
    ...
    def reset_device_info(self):
        """선택한 시리얼의 디바이스 정보 초기화"""
        selected = self.tree.selection()
        if not selected:
            messagebox.showwarning("경고", "초기화할 시리얼을 선택해주세요.")
            return
        
        if not messagebox.askyesno("확인", 
            f"{len(selected)}개 시리얼의 디바이스 정보를 초기화하시겠습니까?\n\n"
            "• 디바이스 정보: 비움\n"
            "• 상태: 사용가능\n"
            "• 활성화 횟수: 0"):
            return
        ...
        try:
            for item in selected:
                ...
                try:
                    ...
                    if response.status_code == 200:
                        logging.info(f"서버 디바이스 정보 초기화 성공: {serial}")
                    else:
                        logging.warning(f"서버 업데이트 실패: {response.status_code}")
                except Exception as e:
                    logging.warning(f"서버 업데이트 실패: {e}")
            
            self.refresh_serial_list()
            messagebox.showinfo("완료", f"{len(selected)}개 시리얼의 디바이스 정보가 초기화되었습니다.")
            
        except Exception as e:
            logging.error(f"디바이스 정보 초기화 오류: {e}")
            messagebox.showerror("오류", f"디바이스 정보 초기화 중 오류가 발생했습니다.\n{str(e)}")
